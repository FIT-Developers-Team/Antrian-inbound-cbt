...
async function stateDelta(since: string): Promise<Record<string, unknown>> {
  ...
  const [
    ...
    { data: inboundMp, error: checkerError },
  ] = await Promise.all([
    db
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (request.method === "POST" && action === "update_ticket_status") {
      return jsonResponse(request, 200, {
        ok: true,
        data: await rpc("inbound_update_ticket_status", {
          p_payload: body,
          p_actor: actor,
        }),
      });
    ...
    if (
      ...
      return jsonResponse(request, 200, {
        ok: true,
        data: await rpc("inbound_update_ticket_pos", {
          p_action: action,
          p_payload: body,
          p_actor: actor,
        }),
    ...
    if (request.method === "POST" && action === "delete_tickets_by_date") {
      return jsonResponse(request, 200, {
        ok: true,
        data: await rpc("inbound_delete_tickets_by_date", {
          p_operational_date: body.operational_date,
        }),
      });
    }
    ...
    if (request.method === "POST" && action === "bulk_complete_operational") {
      return jsonResponse(request, 200, {
        ok: true,
        data: await rpc("inbound_bulk_complete_operational", {
          p_payload: body,
          p_actor: actor,
        }),
      });
    ...
    if (request.method === "POST" && action === "create_ba") {
      return jsonResponse(request, 201, {
        ok: true,
        data: await rpc("inbound_create_ba", {
          p_payload: body,
          p_actor: actor,
        }),
      });
