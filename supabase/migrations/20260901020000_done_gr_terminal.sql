...
drop trigger if exists ticket_pos_complete_ticket_on_done_gr
on public.ticket_pos;

create trigger ticket_pos_complete_ticket_on_done_gr
after update of gr_status on public.ticket_pos
for each row
when (
  upper(coalesce(old.gr_status, '')) is distinct from
  upper(coalesce(new.gr_status, ''))
)
execute function public.complete_ticket_on_all_done_gr();
...
where t.status not in ('COMPLETED', 'EXPIRED')
  and exists (
    select 1
    from public.ticket_pos p
    where p.ticket_id = t.ticket_id
