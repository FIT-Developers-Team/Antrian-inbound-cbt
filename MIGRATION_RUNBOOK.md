...
- Cloudflare Pages project `antrian-inbound-cbt` serves the existing HTML, CSS, JavaScript, images, and fonts only.
...
3. Configure Edge Function secrets. Never put secret values in Git or Cloudflare Pages.
...
7. Build an explicit static package containing only `index.html`, `style.css`, `js/`, and `assets/`, then deploy it to Cloudflare Pages project `antrian-inbound-cbt` on branch `main`.
...
- Cloudflare Pages deployment contains no `api/`, `supabase/`, `.env*`, or package/runtime backend files.
...
- Keep the previous Cloudflare Pages production deployment available until Supabase freshness and UI checks pass.
- If Supabase validation fails, do not promote the static-only deployment.
- If a promoted frontend fails, roll back to the previous immutable Cloudflare Pages deployment; the backend remains on Supabase.
- Do not delete legacy Vercel configuration or disable a still-working legacy scheduler until the replacement is proven fresh.

## Current external blockers (2026-08-24)

- Legacy Vercel hosting is paused and no longer serves production. Production frontend is `https://antrian-inbound-cbt.pages.dev` on Cloudflare Pages.
